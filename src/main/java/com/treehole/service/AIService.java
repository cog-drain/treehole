...
public class AIService {
     ...
     * 根据内容情绪匹配背景白噪音
     */
    public String generateMoodBgm(String content) {
        try {
            org.springframework.http.HttpHeaders headers = new org.springframework.http.HttpHeaders();
            headers.set("Authorization", "Bearer " + API_KEY);
            headers.set("Content-Type", "application/json");
            ...
            Map<String, Object> systemMessage = Map.of(
                    "role", "system",
                    "content", "分析用户文字的情绪，从以下5个关键词中选出一个最匹配的返回，不要返回其他任何文字：[RAIN, FIRE, FOREST, WIND, LOFI]");
            Map<String, Object> userMessage = Map.of("role", "user", "content", content);
            ...
            String response = restTemplate.postForObject(API_URL, entity, String.class);
            JsonNode root = objectMapper.readTree(response);
            String mood = root.path("choices").get(0).path("message").path("content").asText().toUpperCase();

            return switch (mood) {
                case "RAIN" -> "https://commondatastorage.googleapis.com/codeskulptor-assets/sounddogs/rain.mp3"; 
                case "FIRE" -> "https://commondatastorage.googleapis.com/codeskulptor-assets/sounddogs/fireplace.mp3"; 
                case "FOREST" -> "https://commondatastorage.googleapis.com/codeskulptor-assets/cc-birds.mp3"; 
                case "WIND" -> "https://commondatastorage.googleapis.com/codeskulptor-assets/sounddogs/wind.mp3"; 
                default -> "https://commondatastorage.googleapis.com/codeskulptor-assets/Epoq-Lepidoptera.mp3"; 
            };
        } catch (Exception e) {
            return "https://commondatastorage.googleapis.com/codeskulptor-assets/Epoq-Lepidoptera.mp3";
        }
    }
