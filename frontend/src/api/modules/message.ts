...
export const messageApi = {
    // 分页获取留言 (支持 tag 过滤)
    getMessages: (params: PageParams) =>
        request.get<unknown, ApiResponse<PageResult<Message>>>('/messages', { params }),
    ...
    // 表情回应
    reactToMessage: (id: Id, emoji: string) =>
        request.post<unknown, ApiResponse<unknown>>(`/messages/${id}/reactions`, null, { params: { emoji } }),
    ...
    getTrendingTags: (limit = 10) =>
        request.get<unknown, ApiResponse<TrendingTag[]>>('/tags/trending', { params: { limit } })
}
