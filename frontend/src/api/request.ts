...
import type { ApiErrorResponse, ApiResponse, UserIdentity } from '@/types'
...
type ApiResponseInterceptor = {
    use: (
        ...
        onRejected: (error: AxiosError<ApiErrorResponse>) => Promise<never>
    ) => void
}
...
;(request.interceptors.response as unknown as ApiResponseInterceptor).use(
    response => {
        ...
        if (res.code !== undefined && res.code !== 200 && res.code !== 0) {
            const message = getApiErrorMessage(res, '请求失败')
    ...
    (error: AxiosError<ApiErrorResponse>) => {
        if (!window.navigator.onLine || error.message === 'Network Error' || error.code === 'ECONNABORTED') {
            ...
            ElMessage.error(getApiErrorMessage(res, '服务响应异常'))
        }
