...
describe('home realtime helpers', () => {
    ...
    it('inserts new messages and removes optimistic placeholders', () => {
        ...
        const result = insertRealtimeMessage(
            [optimistic, feedMessage(2)],
            { id: 1, content: 'new', userId: 'u2' },
            'u1',
            2
        )
    ...
    it('appends new comments once and updates ownership', () => {
        ...
        const duplicate = appendRealtimeComment(
            messages,
            { id: 10, messageId: 1, content: 'reply', userId: 'u1' },
            'u1'
        )
    ...
    it('applies message and comment reaction updates', () => {
        ...
        applyRealtimeReaction(messages, 'COMMENT_REACTION_UPDATE', {
            messageId: 1,
            commentId: 10,
            reactions: { smile: 1 }
        })
