        ...
        <div class="space-y-6 relative z-10">
          <div class="space-y-3">
            ...
            <div v-if="recoveryKey" class="group relative flex items-center gap-2 p-5 bg-blue-500/5 rounded-2xl border border-blue-500/10 font-mono text-sm text-blue-600 transition-all hover:bg-blue-500/10">
              ...
              <button @click="$emit('copy-key')" class="p-2 hover:bg-blue-500/20 rounded-xl transition-all active:scale-90 text-blue-500 relative z-10 cursor-pointer flex-shrink-0">
                <Copy :size="16" />
              </button>
            </div>
            <button v-else @click="$emit('backup')" class="w-full py-5 rounded-2xl bg-gradient-to-r from-blue-600 to-indigo-600 hover:shadow-lg hover:shadow-blue-600/20 text-white font-bold text-xs tracking-widest uppercase transition-all active:scale-95">
              生成备份密钥
            </button>
          </div>
