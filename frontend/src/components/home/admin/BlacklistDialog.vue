...
<template>
  <el-dialog :model-value="visible" @update:model-value="$emit('update:visible', $event)" width="min(95vw, 600px)" :show-header="false" custom-class="glass-dialog">
    <div class="py-6 space-y-8">
      ...
      <button @click="$emit('update:visible', false)" class="w-full py-4 rounded-xl text-[10px] font-bold text-slate-500 hover:text-white transition-colors uppercase tracking-widest">Close Registry</button>
    </div>
  </el-dialog>
</template>
