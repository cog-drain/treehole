...
<template>
  <Transition name="fade">
    <div v-if="visible" class="fixed inset-0 z-[10000] flex items-center justify-center bg-slate-950/40 backdrop-blur-[80px]">
      <div class="w-full max-w-md p-12 space-y-12 text-center animate-in fade-in zoom-in-95 duration-700">
        ...
        <div class="flex flex-col gap-4 pt-8">
          ...
          <button @click="$emit('close')" class="text-[9px] uppercase tracking-[0.2em] text-white/20 hover:text-white/60 transition-colors">Abort Connection</button>
        </div>
      </div>
    </div>
  </Transition>
