    ...
    <div class="flex flex-col gap-4 pt-6 border-t border-white/5">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div class="flex flex-wrap items-center gap-2 min-w-0">
                <label
                    class="flex items-center gap-1.5 px-2.5 sm:px-3 py-2 rounded-full bg-white/5 border border-white/10 text-[11px] text-slate-400 hover:bg-white/10 hover:text-slate-200 cursor-pointer transition-all active:scale-95"
                >
                    <ImagePlus :size="14" />
                    <span class="whitespace-nowrap">{{
                        composeState.imagePreview ? (composeState.isMobile ? '换图' : '更换图片') : '图片'
                    }}</span>
                ...
                <button
                    ...
                    :class="
                        composeState.isConfessionMode
                            ? 'bg-amber-500/15 border-amber-500/30 text-amber-600 shadow-sm shadow-amber-500/10'
            ...
            <div class="flex w-full sm:w-auto items-center gap-3">
                ...
                >
                    <div class="relative">
                        <Archive
                            :size="18"
                            :class="{ 'animate-bounce': !composeState.isOnline && composeState.offlineQueueCount > 0 }"
                        />
                        <span
                            v-if="composeState.offlineQueueCount > 0"
                            class="absolute -top-1 -right-1 w-2 h-2 bg-amber-500 rounded-full animate-pulse"
                        ></span>
                    </div>
                </button>
                ...
                <button
                    ...
                    :class="
                        composeState.isConfessionMode && composeState.isOnline
                            ? 'bg-amber-600 hover:bg-amber-500 shadow-lg shadow-amber-600/20 text-white'
                            : composeState.isOnline
                ...
                >
                    ...
                    <span
                        v-else-if="composeState.isConfessionMode || composeState.isMidnight"
