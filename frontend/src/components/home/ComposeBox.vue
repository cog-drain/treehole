...
<template>
    <section
        ...
        :class="[
            ...
            {
                'opacity-20 blur-[20px] pointer-events-none scale-95':
                    composeState.isZenMode || composeState.adminLoginVisible
            }
        ]"
    >
        <div class="space-y-8">
                    ...
                    <div class="flex items-center p-1.5 gap-3">
                        ...
                        >
                            <div
                                ...
                                :class="
                                    composeState.form.theme === theme.value
                                        ? 'bg-white/20 scale-150 animate-pulse'
                                        : 'bg-transparent scale-100 group-hover/dot:bg-white/10 group-hover/dot:scale-125'
                                "
                            ></div>
                            <div
                                ...
                                :class="[
                                    composeState.form.theme === theme.value
                                        ? 'scale-110 border-white theme-dot-active'
                                        : 'border-white/20 opacity-40 group-hover/dot:opacity-100',
                ...
                <textarea
                    ...
                    :placeholder="
                        composeState.isConfessionMode
                            ? '这里只有神父能听见...'
                            : '说点什么吧……你的秘密在这里很安全 🤫 (支持 Ctrl+Enter 发送)'
                    "
                    maxlength="500"
                    rows="4"
            ...
            <TransitionGroup name="page">
                <div v-if="composeState.imagePreview" key="img" class="relative group/img inline-block mt-4">
                    <img
                        :src="composeState.imagePreview"
                        class="w-24 h-24 object-cover rounded-xl border border-white/10"
                    />
                    <button
                        class="absolute -top-2 -right-2 w-6 h-6 bg-red-500 rounded-full flex items-center justify-center text-white text-xs opacity-0 group-hover/img:opacity-100 transition-opacity"
