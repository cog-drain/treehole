...
<template>
    ...
    >
        <div class="py-6 space-y-6">
            <div class="flex items-center justify-between">
                <div class="space-y-1">
                    ...
                    <p class="text-[10px] text-slate-500 uppercase">
                        Offline Message Buffer ({{ offlineQueueCount }} items)
                    </p>
                </div>
                <button
                    v-if="isOnline"
            ...
            <div class="max-h-[350px] overflow-y-auto space-y-3 pr-2 custom-scrollbar">
                ...
                >
                    <div class="flex justify-between items-start mb-2">
                        <span class="text-[10px] font-bold text-blue-400/60 uppercase tracking-tighter">{{
                            item.authorAlias || '匿名访客'
                        }}</span>
                    ...
                    <div class="flex items-center justify-between text-[9px] text-slate-500 font-mono">
                        ...
                        <span class="px-2 py-0.5 rounded-full bg-slate-800 border border-white/5 uppercase">{{
                            item.theme
                        }}</span>
                    </div>
                </div>
            ...
            <div
                v-if="!isOnline"
                class="p-4 bg-amber-500/5 border border-amber-500/10 rounded-2xl flex gap-3 items-start"
            >
