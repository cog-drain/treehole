        ...
        <div class="flex items-center gap-4">
            ...
            <div class="flex flex-col min-w-0">
                <div class="flex items-center gap-2">
                    <span
                        class="text-sm font-bold tracking-tight text-slate-200 truncate max-w-[120px] sm:max-w-none"
                        >{{ msg.authorAlias || '匿名用户' }}</span
                    >
                    <span
                        v-if="isConfession"
                        class="text-[10px] px-1.5 py-0.5 rounded-md bg-amber-500/10 text-amber-700 border border-amber-500/20 flex items-center gap-0.5"
                    >
                        <span class="text-xs">🕯️</span>告解
                    </span>
                    <span
                        v-if="toneInfo"
                        class="text-[10px] px-1.5 py-0.5 rounded-md bg-white/5 text-slate-400 flex items-center gap-0.5"
                    >
                        <span class="text-xs">{{ toneInfo.emoji }}</span
                        >{{ toneInfo.label }}
                    </span>
                </div>
