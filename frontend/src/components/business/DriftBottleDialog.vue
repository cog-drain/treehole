  ...
  >
    ...
    >
      ...
      <div class="relative z-10 text-center mb-8">
        <el-text 
          class="!text-[9px] tracking-[0.8em] uppercase block mb-1 drop-shadow-sm !text-slate-400"
        >Etheral Sea</el-text>
      ...
      <div v-if="state === 'throwing'" class="relative z-10 space-y-6 animate-in slide-in-from-bottom-4 duration-500">
        <div 
          class="p-1 rounded-2xl border bg-black/[0.03] border-black/5"
        >
          <el-input
            v-model="newContent"
            type="textarea"
      ...
      <div v-if="state === 'picking'" class="relative z-10 py-16 flex flex-col items-center gap-8 animate-in fade-in duration-700">
        <div class="relative">
          <Waves class="w-20 h-20 animate-pulse scale-150 text-cyan-500/20" />
          <div class="absolute inset-0 flex items-center justify-center">
            <div 
              class="w-2 h-2 rounded-full animate-ping bg-cyan-600"
            ></div>
          </div>
        </div>
        <el-text 
          class="!text-xs tracking-[0.5em] animate-pulse !text-slate-400"
        >正在深海搜寻缘分...</el-text>
        <el-button 
          link 
      ...
      <div v-if="(state === 'picked' || state === 'reply') && pickedData" class="relative z-10 space-y-6 animate-in zoom-in-95 duration-500">
        <div 
          class="flex items-center justify-between p-4 rounded-2xl border bg-black/[0.02] border-black/5"
        >
          <div class="flex items-center gap-4">
            <div 
              class="w-12 h-12 rounded-xl flex items-center justify-center text-2xl bg-white shadow-sm"
            >✉️</div>
            <div>
        ...
        <Transition name="page">
          <div v-if="state === 'reply'" class="space-y-4 animate-in slide-in-from-bottom-4 duration-500">
            <div 
              class="glass-card !p-1 overflow-hidden bg-black/5 border-black/5"
            >
              <el-input
      ...
      <div v-if="state === 'my-bottles'" class="relative z-10 space-y-4 animate-in slide-in-from-left-4 duration-500">
        <div class="flex items-center gap-2 mb-2">
          <el-button link @click="state = 'init'" class="!text-slate-500">
