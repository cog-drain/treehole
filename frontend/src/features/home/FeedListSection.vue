    ...
    <div v-if="total > 0" class="space-y-12">
      <TransitionGroup name="msg-list">
        <MessageCard
          ...
          @like="$emit('like', msg)"
          @toggle-comments="$emit('toggle-comments', msg)"
          @delete="$emit('delete', msg)"
          @delete-comment="$emit('delete-comment', $event)"
          @publish-comment="$emit('publish-comment', msg)"
          @react-comment="$emit('react-comment', $event)"
        ...
        <nav class="flex items-center gap-1 p-2 bg-slate-900/40 backdrop-blur-2xl border border-white/10 rounded-full shadow-[0_0_50px_-12px_rgba(0,0,0,0.5)] ring-1 ring-white/5">
          ...
          <div class="flex items-center gap-1">
            <button
              ...
              :class="pageNum === page ? 'text-white' : 'text-slate-500 hover:text-slate-200'"
            >
              <div
                v-if="pageNum === page"
